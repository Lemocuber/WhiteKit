...
- GitHub Actions builds macOS, Windows x64, and Windows ARM test artifacts on pushes to `dev`; macOS artifacts ship as a zipped unsigned `WhiteKit.app`, Windows artifacts ship as zipped portable `whitekit.exe` builds named `whitekit-windows.zip` and `whitekit-windows-arm.zip`
