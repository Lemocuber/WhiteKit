...
- GitHub Actions builds macOS and Windows artifacts on pushes to `zh` without npm tests; the `dev` branch keeps a separate test-enabled build trigger. `zh` zip artifacts are named `whitekit-macos-zh.zip` and `whitekit-windows-zh.zip`
