...
- GitHub Actions builds macOS, Windows x64, and Windows ARM artifacts on pushes to `zh` without npm tests; the `dev` branch keeps a separate test-enabled build trigger. `zh` zip artifacts are named `whitekit-macos-zh.zip`, `whitekit-windows-zh.zip`, and `whitekit-windows-arm-zh.zip`
